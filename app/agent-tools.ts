import {conceptMatches,type Atlas,type Concept} from './anatomy';
...
export function atlasTools(atlas:Atlas,inspect:(concept:Concept)=>void):Tool[]{return [
 {name:'find_anatomy',description:'Find anatomical structures by name, clinical alias, or source atlas identifier in this atlas.',inputSchema:{type:'object',properties:{query:{type:'string',minLength:1}},required:['query'],additionalProperties:false},annotations:{readOnlyHint:true},execute(input){const data=record(input);if(typeof data.query!=='string'||!data.query.trim())throw new Error('A nonempty query is required.');return atlas.concepts.filter(c=>conceptMatches(c,data.query as string)).slice(0,30).map(c=>({id:c.id,name:c.name,pieces:c.elements.length}));}},
