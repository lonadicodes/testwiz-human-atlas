...
Adaptations: native meter/Y-up coordinates and node transforms are baked into the stage, source hierarchy and identifiers are retained as concepts, coincident vertices are welded and source normals averaged before simplification, geometry is simplified with a 0.2% per-structure relative error bound, normals are quantized to signed 16-bit, and chunks are gzip-compressed. The female source is a whole-body surface with selected organs and incomplete skeletal/muscular coverage; it is not a complete model of every human structure or a single-person scan. Female reproductive anatomy remains in the normal Reproductive layer; pregnancy reference and body surface are hidden initially. No male-derived bones or second-donor muscles are included.
